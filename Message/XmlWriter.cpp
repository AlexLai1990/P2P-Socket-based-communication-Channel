...
void main(){
  ...
  if(out.good()){ out << wtr.xml().c_str(); out.close(); }
  std::cout << std::endl;
  ...
  std::cout << "\n  " << bcwtr1.xml();
  bcwtr1.clear();
  std::cout << "\n  " <<cwtr.start("parent").addBody(bcwtr1.element("child1","child1's body").xml()).addBody(bcwtr2.element("child2","body2").xml().end().xml();
}
#endif
